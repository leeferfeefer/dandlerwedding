...
const readFilesFrom = (directory, callback) => {
  fs.readdirSync(directory).sort((a, b) => {
...
const writeImportStatements = () => {
  console.log("Writing import statements...");
  let count = 0;


  const writeStatement = (fileName) => {
    const content = `import image${count} from "./${fileName}"; \n`;

    // flag a - open the file for writing, positioning the stream at the end of the file. The file is created if it does not exist
    try {
      fs.writeFileSync("./index.js", content, { flag: 'a' });
    } catch (err) {
      console.error("FAILURE TO WRITE: " + err);
    }
    count++;
  };


  readFilesFrom("./src/images/Gallery", (fileName) => {
    // writeStatement(fileName);
    writeSizeMapping(fileName);
  });
  writeExportStatement(count);
};

const writeExportStatement = (count) => {
  console.log("Writing export statement...");
...
const writeSizeMapping = (fileName) => {
  console.log("Writing size mappings...");
  
  const getFileNumber = (testStr) => {
    return testStr.substring(testStr.lastIndexOf("/") + 1, testStr.indexOf("."));
  }

  GalleryPhotos.find((photoMapping, index) => {
    if (fileName.split(".")[0] === getFileNumber(photoMapping.src)) {
      // photoMapping.src = 
    }
  })
};
